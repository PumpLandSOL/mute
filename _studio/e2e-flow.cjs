...
const B = 'http://localhost:8220'; const W = '0x00000000000000000000000000000000000000b2';
...
(async () => {
  ...
  ok('mint 500 mUSD costs 500 USDG total', mt.ok && f(mt.usdg) === 500 && f(mt.musd) === 500, `usdg=${f(mt.usdg)} musd=${f(mt.musd)} collat=${f(mt.usedUsdg)} algo=${f(mt.algoUsd)}`);
  ...
  const st = await post('/api/stake', { amount: 200 }); ok('stake 200', st.ok && f(st.musd) === 300 && f(st.happy.staked) === 200, JSON.stringify(st.happy));
  await new Promise((r) => setTimeout(r, 3000));
  const ac = await post('/api/account', {}); ok('reward accrues in mUSD terms', ac.happy.accruedUsd > 0 && ac.happy.accruedMute > 0, `accruedUsd=${ac.happy.accruedUsd.toExponential(2)} accruedMute=${f(ac.happy.accruedMute)}`);
  const cl = await post('/api/claim', {}); ok('claim gated below $0.01 or pays MUTE', cl.error === 'nothing to claim yet' || (cl.ok && cl.claimedMute > 0), cl.error || 'claimed ' + f(cl.claimedMute));
  const us = await post('/api/unstake', { amount: 200 }); ok('unstake 200 minus 30bps service charge', us.ok && f(us.unstaked) === 199.4 && f(us.fee) === 0.6, `back ${f(us.unstaked)} musd=${f(us.musd)}`);
  const rd = await post('/api/redeem', { amount: 100 }); ok('redeem 100 mUSD pays CR in USDG (net of 50bps)', rd.ok && rd.gotUsdg > 0, `usdg +${f(rd.gotUsdg)} mute +${f(rd.gotMute)} now usdg=${f(rd.usdg)}`);
