// QUIET YIELD E2E (dev server, DEV_FAUCET=1, fresh DATA_PATH): fee share streams to muted holders pro-rata, conserved, only from real wallets.
const B = 'http://localhost:' + (process.env.PORT || 8220); const A = '0x00000000000000000000000000000000000000f1', C = '0x00000000000000000000000000000000000000f2', D = '0x00000000000000000000000000000000000000f3';
...
(async () => {
  ...
  // A mutes 3000 first: A is not yet a holder when the fee is charged, no holders -> nothing paid
  const a1 = await post('/api/shield', A, { amount: 3000 }); ok('first mute: no holders yet, nothing streamed', near(a1.priv, 3000 * 0.997) && a1.quietEarned === 0, 'priv ' + a1.priv);
  // C mutes 1000: fee 3, 40% = 1.2 -> all to A (only holder at that moment)
  const c1 = await post('/api/shield', C, { amount: 1000 }); const a2 = await post('/api/account', A, {});
  ok('C mute fee streams 40% to A', near(a2.quietEarned, 1.2) && near(a2.priv, 2991 + 1.2), 'A earned ' + a2.quietEarned);
  ok('C not paid from own entry fee', c1.quietEarned === 0 && near(c1.priv, 997));
  // D sends nothing, just mutes 2000: fee 6, 40% = 2.4 split A:C by balance
  ...
  // earned mUSD is real: A unmutes everything and redeems
  const u = await post('/api/unshield', A, { amount: a3.priv }); ok('earned mUSD unmutes like any other', u.ok && u.musd > 2000 + 2980, 'musd ' + u.musd.toFixed(2));
